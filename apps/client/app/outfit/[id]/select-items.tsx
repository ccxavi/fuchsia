...
import { getClothingItems, getOutfit, addItemToOutfit, removeItemFromOutfit, ClothingItemResponse, OutfitWithWardrobesResponse } from '@/api/client';
...
export default function SelectItemsScreen() {
  ...
  useEffect(() => {
    async function loadData() {
      if (!id) return;
      try {
        const [outfitData, allItems] = await Promise.all([
        ...
        const existingIds = new Set(outfitData.clothing_items.map(i => i.id));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const itemId of added) {
        promises.push(addItemToOutfit(id as string, itemId));
      }
      
      for (const itemId of removed) {
        promises.push(removeItemFromOutfit(id as string, itemId));
      }
        ...
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerCenter}>
            {outfit?.images && outfit.images.length > 0 ? (
              <Image source={{ uri: outfit.images[outfit.images.length - 1].image_url }} style={styles.headerImage} contentFit="cover" />
            ) : (
